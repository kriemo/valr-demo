...
bed_glyph <- function(x, y = NULL, pass_arg = NULL,
                      .fun, 
                      x_name = "X",
                      y_name = "Y",
                      start_col = "start",
                      end_col = "end",
                      function_title_commands = "(x, y)",
                      ...) {
  ...
  } else if (!is.null(pass_arg)){
    x <- mutate(x, bin = 1, title = x_name)
    res <- eval(.fun(pass_arg, ...)) 
    res <- mutate(res, bin = 1, 
                  title = "Result") 
    comb <- bind_rows(x, res) 
  ...
  p_title = paste0(substitute(.fun), function_title_commands)
  plot_glyph(comb, p_title)
}


plot_glyph <- function(comb, p_title){
  ggplot(comb) + 
...
bed_glyph(a, .fun = bed_cluster, max_dist = 20, 
          function_title_commands = "(x, max_dist = 20)") +
...
bed_glyph(a, .fun = bed_complement, genome = g_genome, function_title_commands = "(x)")
```
...
bed_glyph(a, .fun = bed_merge, function_title_commands = "(x)")
```
...
bed_glyph(a, .fun = bed_flank, genome = g_genome, both = 20, function_title_commands = "(x, both = 20)")
```
...
bed_glyph(a, .fun = bed_slop, genome = g_genome, both = 20, 
          function_title_commands = "(x, both = 20)")
```
...
bed_glyph(a, .fun = bed_shift, genome = g_genome, size = -20, 
          function_title_commands = "(x, size = -20)")
```
...
bed_glyph(a, b, start_col = "start.x", end_col = "end.x", .fun = bed_intersect)
```
...
bed_glyph(a, b, start_col = "start.x", end_col = "end.x", .fun = bed_window, genome = g_genome, both = 15, 
          function_title_commands = "(x, y, both = 15)")
```
...
bed_glyph(a, x_name = "Genome", pass_arg = g_genome, .fun = bed_random, 
          length = 10, 
          n = 3,
          seed = seed, 
          function_title_commands = "(x, length = 10, n = 3)")
```
